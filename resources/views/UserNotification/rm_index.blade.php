      ...
      <table class="table table-hover mb-6 font-size-h5">
         ...
         <tbody>
            ...
            <tr>
               ...
               <td>{{ en2bn($row->rm_case_rgister->case_no) ?? '' }}</td>
               <td>{{ en2bn($row->rm_case_rgister->case_date) ?? '' }}</td>
               <td class="text-danger">
                   {{ en2bn(App\Models\RM_CaseHearing::select('hearing_date')->orderby('id', 'DESC')->where('rm_case_id', $row->rm_case_id)->first()->hearing_date ?? '') }}
                   {{-- {{ en2bn($row->hearing_date) ?? '' }} --}}
                </td>
               <!-- <td>{{ $row->rm_case_rgister->court->court_name ?? '' }}</td> -->
               <td>{{ $row->rm_case_rgister->upazila->upazila_name_bn ?? '' }}</td>
               <td>{{ $row->rm_case_rgister->mouja->mouja_name_bn ?? '' }}</td>
               <td>
                   <a class="btn btn-primary btn-sm" href="{{ route('rmcase.show', $row->rm_case_rgister->id) }}">বিস্তারিত</a>
               </td>
            </tr>
            @endforeach
         </tbody>
